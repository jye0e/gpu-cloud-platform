...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-6">
      {/* 欢迎横幅 */}
      <div className="bg-gradient-to-r from-brand-600 to-indigo-600 rounded-2xl p-6 text-white relative overflow-hidden">
        ...
        <div className="relative">
          ...
          <div className="flex gap-3">
            <Link to="/upload" className="inline-flex items-center gap-2 px-4 py-2 bg-white/15 hover:bg-white/25 backdrop-blur rounded-lg text-sm font-medium transition-all">
              <Upload className="w-4 h-4" />
              上传模型
            </Link>
            <Link to="/inference" className="inline-flex items-center gap-2 px-4 py-2 bg-white/15 hover:bg-white/25 backdrop-blur rounded-lg text-sm font-medium transition-all">
            ...
            <Link key={i} to={stat.link}>
              <Card hover className="p-5">
                ...
                <p className="text-2xl font-bold text-slate-800">
                  {stat.value}
                  {stat.total !== undefined && <span className="text-base text-slate-400 font-normal">/{stat.total}</span>}
                  {stat.unit && <span className="text-base text-slate-400 font-normal">{stat.unit}</span>}
                </p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="lg:col-span-2 p-5">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-semibold text-slate-800">部署服务</h3>
            <Link to="/services" className="text-sm text-brand-600 hover:underline flex items-center gap-1">
          ...
          {services.length === 0 ? (
            <EmptyState
              ...
              action={<Link to="/deploy" className="text-sm text-brand-600 hover:underline">去部署 →</Link>}
            />
          ) : (
            <div className="space-y-3">
              {services.slice(0, 5).map(s => (
                <div key={s.service_id} className="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                  <div className="flex items-center gap-3 min-w-0">
                    <div className={`w-2 h-2 rounded-full ${s.status === 'running' ? 'bg-emerald-500' : 'bg-slate-400'}`} />
                    <div className="min-w-0">
                      <p className="text-sm font-medium text-slate-700 truncate">{s.service_name}</p>
                      <p className="text-xs text-slate-400 truncate">{s.model_name}</p>
                    </div>
                  </div>
        ...
        {/* 资源详情 */}
        <Card className="p-5">
          <h3 className="font-semibold text-slate-800 mb-4">资源详情</h3>
          <div className="space-y-4">
            ...
            <div className="pt-3 border-t border-slate-100">
              ...
              <div className="space-y-1.5 text-xs">
                <div className="flex justify-between">
                  <span className="text-slate-400">显存利用率上限</span>
                  <span className="text-slate-600">{Math.round((resource?.gpu_config?.gpu_memory_util_limit || 0) * 100)}%</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-400">最大上下文</span>
                  <span className="text-slate-600">{resource?.gpu_config?.max_model_len || 0} tokens</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-400">QPS 限制</span>
                  <span className="text-slate-600">{resource?.qps_limit || 0}</span>
                </div>
              </div>
            </div>
            ...
            <div className="pt-3 border-t border-slate-100 space-y-2">
              <Link to="/upload" className="flex items-center justify-between p-2.5 bg-brand-50 hover:bg-brand-100 rounded-lg transition-colors">
                <span className="text-sm text-brand-700 font-medium flex items-center gap-2">
                  <Upload className="w-4 h-4" /> 上传新模型
                </span>
                <ArrowRight className="w-3.5 h-3.5 text-brand-400" />
              </Link>
              <Link to="/deploy" className="flex items-center justify-between p-2.5 bg-emerald-50 hover:bg-emerald-100 rounded-lg transition-colors">
