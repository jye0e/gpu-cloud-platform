  ...
  return (
    <div className="flex flex-col h-[calc(100vh-8rem)] max-w-4xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          <h2 className="text-lg font-semibold text-slate-800 mb-1">推理测试</h2>
          <p className="text-sm text-slate-500">标准 OpenAI 兼容接口在线测试</p>
        </div>
      ...
      {services.length === 0 ? (
        <Card className="flex-1 flex items-center justify-center">
          <EmptyState
            ...
            action={
              <a href="/deploy" className="text-sm text-brand-600 hover:underline">去部署 →</a>
            }
          />
        </Card>
      ) : (
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              {messages.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-center py-12">
                  ...
                  <p className="text-sm font-medium text-slate-600">开始对话</p>
                  <p className="text-xs text-slate-400 mt-1">输入消息，测试模型推理效果</p>
                </div>
              ) : (
                messages.map((msg, i) => (
                  <div key={i} className={`flex gap-3 ${msg.role === 'user' ? 'justify-end' : ''}`}>
                    {msg.role === 'assistant' && (
                      <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${msg.error ? 'bg-red-100' : 'bg-brand-100'}`}>
                        <Bot className={`w-4 h-4 ${msg.error ? 'text-red-500' : 'text-brand-600'}`} />
                      </div>
                    )}
                    <div className={`max-w-[70%] px-4 py-2.5 rounded-2xl text-sm ${
                      msg.role === 'user'
                        ...
                        : msg.error
                          ? 'bg-red-50 text-red-600 rounded-tl-md'
              ...
              {sending && (
                <div className="flex gap-3">
                  ...
                  <div className="bg-slate-100 px-4 py-2.5 rounded-2xl rounded-tl-md">
            ...
            <div className="border-t border-slate-200 p-3">
              <div className="flex items-end gap-2">
                <textarea
                  ...
                  rows="1"
                  className="flex-1 px-3.5 py-2.5 rounded-xl border border-slate-300 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-transparent resize-none max-h-32"
